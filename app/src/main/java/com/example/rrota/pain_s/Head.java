...
/**
 * Created by Камилла on 29.03.2018.
 */
...
public class Head extends AppCompatActivity implements View.OnClickListener{
    ...
    final String name[] ={"Пульсирующая","Давящая","Стреляющая","Не очень сильная",
            "Длится до 3 суток","Длится до 7 дней","Длится несколько часов","Усиливается в утром",
            "В одной половине головы","В области лба и глаза"," В темени, области лба, может захватывать лицо, зубы ","Во всей голове",
            "Невротические расстройства","Покраснение и отек лица","Слезоточение глаза","Спазм мышц лица","'Мушки' перед глазами","Шум в ушах","Потеря слуха на одно ухо",
            "Повышение температуры","Насморк","Кашель","Тошнота и рвота","Головокружение","Скачки давления",
            "Плохая переносимость яркого света,шума","Ощущаение эмоционального или физического напряжения ","Ощущение тревоги","Ощущение раздражительности","Снижение концентрации внимания"};
    final String question[]={"Характер боли","Боль","Боль ощущается","У Вас","Катаральные симптомы","Симптомы"};
    final String disease[]={"Мигрень","Головная боль напряжения (ГБН)","Кластерная головная боль",
            "Головная и лицевая боль из-за невралгии тройничного нерва","Шейный остеохондроз","Грипп или простуда",
            "Повышенное внутричерепное давление","Сотрясение головного мозга","Вегетососудистая дистония",
            "Гипертоническая болезнь","Синусит, фронтит, гайморит"};
    ...
    public void max(){
        byte m=count[0];
        for(byte i=1;i<11;i++) if (count[i]>m)  m=count[i];
        ...
        Log.d("myLog", k+" max");
        for(byte i=0;i<11;i++) if (count[i]==m)  {s+=disease[i]+" "; count[i]=-1;}
        tx.setText(s);
        tx.setTextSize(30);
        ...
        if ((count[4]==-1))  { if (f) s2+="Вертеброневролог "; else {s2+="\n Также специалисты: "; s2+="Вертеброневролог "; f=true;}}
        if ((count[7]==-1)) { if (f) s2+="Травматолог Нейрохирург"; else {s2+="\n Также специалисты: "; s2+="Травматолог Нейрохирург"; f=true;}}
        if ((count[9]==-1)) { if (f) s2+="Кардиолог "; else {s2+="\n Также специалисты: "; s2+="Кардиолог "; f=true;}}
        if ((count[10]==-1)) { if (f) s2+="Отоларинголог "; else {s2+="\n Также специалисты: "; s2+="Отоларинголог "; f=true;}}
        tx2.setText(s2);
        tx2.setTextSize(30);
        final String d=s;
        final String doc=s2;
